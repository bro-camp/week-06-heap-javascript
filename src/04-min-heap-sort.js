...
class MinHeap {
  ...
  insert(data) {
    ...
    let parentIdx = MinHeap.parent(currIdx);
    ...
    while (parentIdx >= 0 && elms[currIdx] < elms[parentIdx]) {
      ...
      currIdx = parentIdx;
      parentIdx = MinHeap.parent(currIdx);
    }
  }
  ...
  remove() {
    ...
    elms[0] = elms[len - 1];
    let parent = 0;
    let leftChild = MinHeap.leftChild(parent);
    let rightChild = MinHeap.rightChild(parent);
    ...
    while (leftChild < len || rightChild < len) {
      ...
      leftChild = MinHeap.leftChild(parent);
      rightChild = MinHeap.rightChild(parent);
    }
  ...
  toDotString() {
    ...
    for (let i = 1; i < elms.length; i += 1) {
      edgeDotStr = `${edgeDotStr}  ${MinHeap.parent(i)} -- ${i};\n`;
    }
  ...
  async createGraph() {
    const fileBaseName = '04-min-heap-sort';
...
function testHeapSort() {
  ...
  console.log(`[${ar}]`);
  const descSortedArray = heapSortAsc(ar);
